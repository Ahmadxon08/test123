...
const LikedItems = ({ likedItems }) => {
  ...
  const toggleLiked = (index) => {
    if (Object.prototype.hasOwnProperty.call(likedList, index)) {
      // The index exists in likedList
      if (likedList[index]) {
        // If liked, remove it from the liked list
        const updatedLikedList = { ...likedList };
        delete updatedLikedList[index];
        setLikedList(updatedLikedList);
      } else {
        // If not liked, add it to the liked list
  ...
  const filteredPlaylists = likedItems.filter((playlist) =>
    ...
    <>
      ...
      <div className="play_list">
        {filteredPlaylists.length === 0 ? (
          <h1 className="likedNot">No Liked Song Yet !!!.</h1>
        ) : (
